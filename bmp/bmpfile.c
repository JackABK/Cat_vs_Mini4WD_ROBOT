...
bmp_malloc_colors(bmpfile_t *bmp)
{
  ...
  if (bmp->dib.depth == 1 || bmp->dib.depth == 4 || bmp->dib.depth == 8) {
    bmp->colors = pvPortMalloc(sizeof(rgb_pixel_t) * bmp->dib.ncolors);
...
bmp_malloc_pixels(bmpfile_t *bmp)
{
  ...
  bmp->pixels = pvPortMalloc(sizeof(rgb_pixel_t *) * bmp->dib.width);
  for (i = 0; i < bmp->dib.width; ++i) {
    bmp->pixels[i] = pvPortMalloc(sizeof(rgb_pixel_t) * bmp->dib.height);
...
bmp_free_pixels(bmpfile_t *bmp)
{
  ...
  for (i = 0; i < bmp->dib.width; ++i)
    vPortFree(bmp->pixels[i]);

  vPortFree(bmp->pixels), bmp->pixels = NULL;
}
...
bmp_create(uint32_t width, uint32_t height, uint32_t depth)
{
  ...
  result = pvPortMalloc(sizeof(bmpfile_t));
  ...
  bytes_per_pixel = (result->dib.depth * 1) / 8;
  bytes_per_line = (int)(bytes_per_pixel * result->dib.width);
  if ((bytes_per_line &0x03) != 0)
    bytes_per_line += 4 - (bytes_per_line&0x03);
...
bmp_write_header(bmpfile_t *bmp, FILE *fp)
{
  ...
  fl_fwrite(header.magic, sizeof(header.magic), 1, fp);
  fl_fwrite(&(header.filesz), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(header.creator1), sizeof(uint16_t), 1, fp);
  fl_fwrite(&(header.creator2), sizeof(uint16_t), 1, fp);
  fl_fwrite(&(header.offset), sizeof(uint32_t), 1, fp);
}
/*
bmpfile_t *bmp_create_from_file(const char *filename){
	bmpfile_t bmp,ret_bmp;

	FILE* fp;
	fp=fopen(filename,"r");

	bmp=malloc(sizeof(bmpfile_t) );
	bmp_read_header(bmp,fp);
	bmp_read_dib(bmp, fp);
	ret_bmp=bmp_create(bmp->dib.width,bmp->dib.height,bmp->dib.depth);
...
bmp_write_dib(bmpfile_t *bmp, FILE *fp)
{
  ...
  fl_fwrite(&(dib.header_sz), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.width), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.height), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.nplanes), sizeof(uint16_t), 1, fp);
  fl_fwrite(&(dib.depth), sizeof(uint16_t), 1, fp);
  fl_fwrite(&(dib.compress_type), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.bmp_bytesz), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.hres), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.vres), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.ncolors), sizeof(uint32_t), 1, fp);
  fl_fwrite(&(dib.nimpcolors), sizeof(uint32_t), 1, fp);
}
...
bmp_write_palette(bmpfile_t *bmp, FILE *fp)
{
  if (bmp->dib.depth == 1 || bmp->dib.depth == 4 || bmp->dib.depth == 8) {
    int i;
    for (i = 0; i < bmp->dib.ncolors; ++i)
      fl_fwrite(&(bmp->colors[i]), sizeof(rgb_pixel_t), 1, fp);
  }
    ...
    fl_fwrite(&red_mask, sizeof(uint16_t), 1, fp);
    fl_fwrite(&zero_word, sizeof(uint16_t), 1, fp);

    fl_fwrite(&green_mask, sizeof(uint16_t), 1, fp);
    fl_fwrite(&zero_word, sizeof(uint16_t), 1, fp);

    fl_fwrite(&blue_mask, sizeof(uint16_t), 1, fp);
    fl_fwrite(&zero_word, sizeof(uint16_t), 1, fp);
  }
}
...
bmp_save(bmpfile_t *bmp, const char *filename)
{
  ...
  if (bmp->dib.depth == 16) {
    ...
    uint32_t padding_bytes = 4 - (data_bytes&0x03);
	...
	fl_fwrite(&value, sizeof(uint16_t), 1, fp);
      }

      for (write_number = 0; write_number < padding_bytes; ++write_number)
	fl_fwrite(&zero_byte, 1, 1, fp);
    }
  }
  else {
    int bytes_per_pixel;
    ...
    bytes_per_pixel = (bmp->dib.depth * 1) / 8;
    bytes_per_line = (int)(bytes_per_pixel * bmp->dib.width);
    if ((bytes_per_line &0x03) != 0)
      bytes_per_line += 4 - (bytes_per_line&0x03);

    buf = pvPortMalloc(bytes_per_line);
	...
	break;
      ...
      fl_fwrite(buf, bytes_per_line, 1, fp);
    }
    vPortFree(buf);
  }
